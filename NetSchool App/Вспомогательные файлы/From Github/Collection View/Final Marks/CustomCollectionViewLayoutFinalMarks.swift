...
class CustomCollectionViewLayoutFinalMarks: UICollectionViewLayout {
...
// MARK: - Helpers
extension CustomCollectionViewLayoutFinalMarks {
    ...
    func sizeForItemWithColumnIndex(_ columnIndex: Int) -> CGSize {
        var text: [NSString] = ["Физическая культура", "1 четверть", "1 четверть", "1 четверть", "1 четверть", "Годовая", "Экзамен", "Итоговая"]
        let length = UserDefaults.standard.object(forKey: "sectionLength") as? Int ?? 0
        var sum: CGFloat = 0
        for k in text{
            sum += (k.size(withAttributes: [NSAttributedStringKey.font: UIFont.systemFont(ofSize: 14.0)]).width + 10)
        }
        var width:CGFloat = columnIndex == 0 ? CGFloat(length) : text[columnIndex].size(withAttributes: [NSAttributedStringKey.font: UIFont.systemFont(ofSize: 14.0)]).width
        width += 10
//        let width: CGFloat = text[columnIndex].size(withAttributes: [NSAttributedStringKey.font: UIFont.systemFont(ofSize: 14.0)]).width + 10
        ...
        return CGSize(width: width*koff, height: 45)
    }
