...
class CustomCollectionViewLayoutProgressWork: UICollectionViewLayout {
...
// MARK: - Helpers
extension CustomCollectionViewLayoutProgressWork {
    ...
    func sizeForItemWithColumnIndex(_ columnIndex: Int) -> CGSize {
        var text: [NSString] = ["Домашняя работа", "", "Дата выполнения", "Балл"]
        var sum: CGFloat = 0
        for k in text{
            sum += (k.size(withAttributes: [NSAttributedStringKey.font: UIFont.systemFont(ofSize: 14.0)]).width + 10)
        }
        sum += (collectionView?.frame.width)!*0.65
        let width: CGFloat
        if columnIndex == 1{
            width = (collectionView?.frame.width)!*0.65
        }else{
            width = text[columnIndex].size(withAttributes: [NSAttributedStringKey.font: UIFont.systemFont(ofSize: 14.0)]).width + 10
        }
